"""
Hirist.tech platform adapter — tech-only niche portal.
Credentials: HIRIST_EMAIL / HIRIST_PASSWORD env vars.
"""
...
class HiristPlatform(BasePlatform):
    name = "hirist"

    def login(self, page) -> None:
        email = os.environ.get("HIRIST_EMAIL", self.profile["email"])
        password = os.environ.get("HIRIST_PASSWORD", "")
        if not password:
            raise RuntimeError("Set HIRIST_PASSWORD env var.")
        ...
        try:
            ...
            pass
        log.info(f"Hirist: login attempted. URL: {page.url}")
    ...
    def _search_one(self, page, keyword: str, city: str) -> list[Job]:
        url = f"{BASE_URL}/jobs/{quote(keyword.lower().replace(' ', '-'))}-jobs-in-{quote(city.lower())}"
        try:
        ...
        jobs = []
        cards = page.query_selector_all('[class*="job-listing"], [class*="jobCard"]')
        for card in cards:
            try:
                title_el = card.query_selector('[class*="title"] a, h3 a, h2 a')
                ...
                continue
        log.info(f"Hirist: '{keyword}' in {city} → {len(jobs)} jobs.")
        return jobs
    ...
    def apply_to_job(self, page, job: Job, resume_path: Path) -> bool:
        ...
        for sel in ['button:has-text("Apply")', 'a:has-text("Apply Now")', '[class*="apply"]']:
            try:
                btn = page.wait_for_selector(sel, timeout=5000)
                if btn and btn.is_visible():
                    btn.click()
                    page.wait_for_timeout(2000)
                    log.info(f"Hirist: applied: {job.title} @ {job.company}")
                    return True
            except PlaywrightTimeoutError:
                continue
        return False
